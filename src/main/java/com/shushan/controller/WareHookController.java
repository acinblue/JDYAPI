...
@Controller
@RequestMapping("/warehook")
public class WareHookController {
	
	@Autowired
	private WareHookService wareService;
	...
	@RequestMapping(value= {"/ware_manifest"}, method= {org.springframework.web.bind.annotation.RequestMethod.POST}, produces = { "text/plain;charset=UTF-8" })
...
//				String rawQuery = uri.getRawPath();
				Map<String, String[]> pramMap = request.getParameterMap();
//				Map<String, String> paramMap = CommonUtil.parseParameter(rawQuery);
	...
	private void handlerData(String payload) {
			...
			if (DATA_CREATE.equals(op)) {
				wareService.add(data);
			}else if (DATA_UPDATE.equals(op)) {
				wareService.update(data);
			}else {
				wareService.delete(data);
		}
	}
}
